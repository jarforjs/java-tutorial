...
module.exports=function (query,request,response) {
    var id = query.id;
    if (id) {
        /*var rs=fs.createReadStream(Db_NAME);
        var str='';
		...
		var finalUsers=filterUsers.sort(function (a, b) {
                ...
                response.end(JSON.stringify({code: 'success', data: {users:finalUsers,totalPage:Math.ceil(filterUsers.length/pageSize),pageNum:pageNum}}));
            }
        });
    }
