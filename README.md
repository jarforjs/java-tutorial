...
- 强制转换可能会有数据丢失，比如把浮点型转化成整形，小数点后面的就会丢失
...
![Integer包装类常用方法](assets/Integer包装类常用方法.jpg 'Integer包装类常用方法')
...
### 范型
1. 集合中的元素，可以是任意类型的对象（确切的说是对象的引用）
   - 如果把某个对象放入集合，则会忽略它的类型，而把它当作Object处理
2. 范型则是规定了某个集合只可以存放特定类型的对象
   - 会在编译期间进行类型检查
   - 因为已经规定了类型，在用的时候可以直接按指定类型获取集合元素（不用在进行诸如类型转换的操作）
